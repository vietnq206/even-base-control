  ...
  while (this->step(TIME_STEP) != -1) {
    ...
    if (!exeCommand.empty() && indexPath <= path.size())
    {
      if(exeCommand.front() == 1)
      {
        ...
        if(decision==0) 
        {
          std::cout<<"R4: Taking point: X "<<path[indexPath].locX<<" and Y "<<path[indexPath].locZ<<std::endl;
          
          ackAskMove == 0 ? ackAskMove = 1: ackAskMove = 0;
      std::cout<<" Ack R4 - 2: "<<ackAskMove<<std::endl;

          forward();

      std::cout<<" Ack R4 - after: "<<ackAskMove<<std::endl;
