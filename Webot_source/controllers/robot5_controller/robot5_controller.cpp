...
void Slave::run() {
  ...
  while (this->step(TIME_STEP) != -1) {
      std::cout<<" Ack R4: "<<ackAskMove<<std::endl;
     if (!messOut.empty() ) { 
    ...
    if (!exeCommand.empty() && indexPath <= path.size())
    {
      if(exeCommand.front() == 1)
      {
        ...
        if(decision==0) 
        {
          std::cout<<"R5: Taking point: X "<<path[indexPath].locX<<" and Y "<<path[indexPath].locZ<<std::endl;
          
          ackAskMove == 0 ? ackAskMove = 1: ackAskMove = 0;
      std::cout<<" Ack R5 - 2: "<<ackAskMove<<std::endl;

          forward();

      std::cout<<" Ack R5- after: "<<ackAskMove<<std::endl;
